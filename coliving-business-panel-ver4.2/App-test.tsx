...
import { AgentPerformanceSummariser, AgentPerformance } from './components/AgentPerformanceSummariser';

import { AIChatAssistant } from './components/AIChatAssistant';
import { DashboardFilters, FilterState } from './components/DashboardFilters';
import { MessageCircle, CheckCircle, AlertTriangle, ThumbsDown, AlertCircle } from 'lucide-react';

const API_BASE = "https://fastapi-app-a4ha.onrender.com/coliving-ai-os/chat-analysis"; // adjust if backend is elsewhere
...
export default function App() {
  ...
  // Sentiment/Emotion distribution states
  ...
  type PerformanceDateRangeType = 'today' | 'week' | 'month' | 'selectMonth' | 'custom';
  const [performanceRange, setPerformanceRange] = useState<PerformanceDateRangeType>('week');
  const [performanceCustomDateRange, setPerformanceCustomDateRange] = useState<{ from?: Date; to?: Date }>({});
  const [performanceSelectedMonth, setPerformanceSelectedMonth] = useState<{ month: number; year: number } | undefined>(undefined);
  ...
  // ----------------------------
  useEffect(() => {
    fetch(`${API_BASE}/agents-performance`)
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
      .then((data: AgentPerformance[]) => {
        console.log("✅ Agents Performance API response:", data);
        setAgentPerformance(data);
      })
      .catch(err => {
        console.error("❌ Error fetching agent performance:", err);
        setError(`Failed to fetch agent performance: ${err.message}`);
      });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-50/30">
      <div className="w-full px-6 py-6 space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <AgentPerformanceSummariser 
            conversations={filteredConversations} // optional fallback
            agentPerformance={agentPerformance}   // API-powered
            selectedRange={performanceRange}
            customDateRange={performanceCustomDateRange}
            selectedMonth={performanceSelectedMonth}
            onRangeChange={setPerformanceRange}
            onCustomDateRangeChange={setPerformanceCustomDateRange}
            onMonthChange={setPerformanceSelectedMonth}
          />
        </div>
